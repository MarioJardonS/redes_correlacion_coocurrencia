...
nombres_familias <- c()
for (i in 1:3){
  nombres_familias <- union(nombres_familias ,as.vector(taxa_centrales_tomate[[i]][, "Rank5"]))
...
for (i in 1:3){
  for (j in 1:6) {
    if (is.element(nombres_familias[j], as.vector(taxa_centrales_tomate[[i]][, "Rank5"] ))){
      data_familias[j,i] <- 1
    }else
    {
      data_familias[j,i] <- 0
...
A nivel familia los análisis con submuestreos aleatorios 

```{r}
for (i in 1:6){
  print(intersect(as.vector(taxa_tomate_aleatorio1[[i]][ , "Rank5"]) , as.vector(taxa_centrales_tomate[[3]][ , "Rank5"]  ) ))
}
```
