...
library(RColorBrewer)
library(ggplot2)
clave_df$Phylum <- as.factor(clave_df$Rank5)
phylum_colors_rel<- colorRampPalette(brewer.pal(8,"Dark2")) (length(levels(clave_df$Rank5)))
relative_plot <- ggplot(data=clave_df, aes(x=Sample, y=Abundance, fill=Rank5))+ 
    geom_bar(aes(), stat="identity", position="stack")+
    scale_fill_manual(values = phylum_colors_rel)
#absolute_plot | 
relative_plot
...
#Means <- c()
Medians <- c()
for (i in row.names(clave@otu_table@.Data)  ){
...
tabla_muestras <- otu_table(as.data.frame(tabla_muestras) , taxa_are_rows = TRUE)
```
...
metadata_otu <- data.frame( ID = row.names(abundance@otu_table@.Data) , Key = key_not_key , row.names = row.names(abundance@otu_table@.Data) )
metadata_otu <- sample_data(metadata_otu)
```

```{r}
#t_abundance <- phyloseq( otu_table = tabla_muestras , sample_data = metadata_otu )
#meta_ord_clave <- ordinate( t_abundance , method = "PCoA", distance = "bray")
#plot_ordination(physeq = t_abundance , ordination = meta_ord_clave , color = "Key")
```

```{r}
vec_sample <- sample(1:13350 , size = 300)
...
```{r}
key_not_key <- c()
for (i in 1:dim(tabla_muestras_sample@.Data)[2]){
  
  if (is.element( colnames(tabla_muestras_sample@.Data)[i]   , row.names(clave@otu_table@.Data)  )   ){
    key_not_key <- c(key_not_key , "key")
  } else {
    key_not_key <- c(key_not_key , "not_key")
  }
}
metadata_otu_sample <- data.frame( ID = colnames(tabla_muestras_sample@.Data) , Key = key_not_key , row.names = colnames(tabla_muestras_sample@.Data) )
metadata_otu_sample <- sample_data(metadata_otu)
```
...
plot_sample
ggsave("./plot_pcoa_otus1" , plot_sample , device = "png")
```
```{r}
key_not_key <- c()
for (i in 1:dim(abundance@otu_table@.Data)[1]){
