...
```{r}
o_table_key_maiz <- otu_table(otus_centrales[[3]][ ,  intersect(row.names(etapa) , colnames(otus_centrales[[3]]))  ] , taxa_are_rows = TRUE)  
t_table_key_maiz <- tax_table(taxa_centrales[[3]])
row.names(t_table_key_maiz@.Data) <- row.names(taxa_centrales[[3]])
o_table_maiz <- otu_table(data[[3]][, intersect(row.names(etapa) , colnames(otus_centrales[[3]])) ], taxa_are_rows = TRUE)
```
