...
#ggsave("abundance_key_otus_medians.png" , plot ,device = "png")
...
```{r}
plot <- ggplot(key_vs_no_key , aes(x = Sample , y= Abundance , color = key_not_key , group = key_not_key))+ #geom_dotplot(binaxis='y', stackdir='center') +
...
etapa <- data.frame( ID = metadata_tomate[ , "ID"], Etapa =  metadata_tomate [ , "Etapa_fenologica"] , Estado = metadata_tomate[ , "Origen"],row.names = metadata_tomate[ , "ID"])
...
```{r}
o_table_key_tomate <- otu_table(otus_centrales[[2]][1:(dim(otus_centrales[[2]])[1]-1) , 1:(dim(otus_centrales[[2]])[2]-4) ] , taxa_are_rows = TRUE)  
t_table_key_tomate <- tax_table(taxa_centrales[[2]])
row.names(t_table_key_tomate@.Data) <- row.names(taxa_centrales[[2]])
o_table_tomate <- otu_table(data[[2]], taxa_are_rows = TRUE)
```
...
plot_pcoa_muestras <- plot_ordination(physeq = phy_tomate, ordination = meta_ord , color = "Etapa")
plot_pcoa_muestras
...
plot_pcoa_muestras <- plot_ordination(physeq = phy_tomate_key, ordination = meta_ord , color = "Etapa")
plot_pcoa_muestras
#ggsave("./tomate_muestras_pcoa.png" , plot_pcoa_muestras , device = "png")
```
...
etapa <- data.frame( ID = metadata_maiz[ , "ID"], Etapa =  metadata_maiz [ , "Etapa_fenologica"] , Estado = metadata_maiz[ , "Origen"],row.names = metadata_maiz[ , "ID"])
...
plot_pcoa_muestras <- plot_ordination(physeq = phy_maiz_key, ordination = meta_ord , color = "Etapa")
plot_pcoa_muestras
#ggsave("./maiz_muestras_pcoa.png" , plot_pcoa_muestras , device = "png")
```
