...
for (i in 1:dim(key_vs_no_key)[1]){
  ...
  if (is.element( keys_vs_no_key[i, "OTU"]   , row.names(phy_chile_key@otu_table@.Data)  )   ){
    key_not_key <- c(key_not_key , "key")
  } else {
    key_not_key <- c(key_not_key , "not_key")
...
keys_vs_no_key <- cbind(keys_vs_no_key , key_not_key)
#medians <- psmelt(otu_table(as.data.frame(medians) , taxa_are_rows = FALSE))
```
...
```{r}
mini <- function(x){
  return(mean_sdl(x)["ymin"])
}
maxi <- function(x){
  return(mean_sdl(x)["ymax"])
}
